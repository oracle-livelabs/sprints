...
23.6        Mar-17-25       Brianna Ambler  Adding support for LiveSQL integration with LiveLabs sprints
*/
...
let main = function () {
    ...
    $(document).ready(function () {
        ...
        $.when(
            ...
            $.getJSON(manifestFileName, function (manifestFile) {
                ...
                //         fetch(url)  // The file is in the same directory as script.js
                //             .then(response => {
                //                 if (!response.ok) {
                ...
                //             })
                //             .then(text => {
                //                 console.log('File content:', text);  // Do something with the loaded file content
                //                 renderContent(text);
                //             })
                //             .catch(error => {
                ...
                //         let htmlContent = content.replace("<LiveSQL-Button>", '<button class="LiveSQL-Button">Try It Now</button>');
                //         console.log("New file content:" + htmlContent);
                //         // document.getElementById('content').innerHTML = htmlContent;
            
                //         // Attach click event to the button
                //         // document.querySelector('.new-button').addEventListener('click', handleButtonClick);
                //     }
                    
                //     // Load the markdown content
                //     loadFile($(this)[0].filename);
                    
                    // Search for the LiveSQL button

                    
                    // If the LiveSQL button exists, 

                        // Concatenate the code between the LiveSQL tags

                        // Encode it

                        // Define the on-click behavior of the button.
    ...
    let convertLiveSQLButtonTags = function (markdownContent) {
        ...
        if (markdownContent.includes('<livesql-button>')) {
            ...
            let liveSQLRegExp = new RegExp(/<livesql>([\s\S|\n]*?)<\/livesql>/gm); // Finds all content between the livesql tag. 
    
            // Find all code wrapped in <livesql> and concatenate it.
            markdownContent = markdownContent.replace(liveSQLRegExp, function (code) {
            ...
            // Add the encoded the SQL code string to the URL.
            console.log('SQL statements to be included in the LiveSQL worksheet: ' + sqlCode);
            let encodedLiveSQLURL = 'https://livesql.oracle.com/next/worksheet?code=' + encodeURIComponent(sqlCode);
            console.log('Encoded LiveSQL URL: ' + encodedLiveSQLURL);

            // Replace <livesql-button> with the actual button.
            markdownContent = markdownContent.replace('<livesql-button>', function (code) {
                code = code.replace('<livesql-button>', '<a href="' + encodedLiveSQLURL + '" target = "_blank"> <button class="livesql-button">Try It Now w/ Live SQL</button></br></a>');
                return code;
            });
            console.log("The Live SQL button is added and all Live SQL tags are removed.")
